...
import { PaginationQueryDto, CreateMenuDto, UpdateMenuDto } from '@cdm/shared';
...
export class MenuProxyController {
  ...
  @Post()
  async create(@Body() body: CreateMenuDto, @CurrentUser() user: IdentityPayload) {
    return firstValueFrom(this.authClient.send({ cmd: 'menu_create' }, { data: body, identity: user }));
  }

  @Put(':id')
  async update(@Param('id') id: string, @Body() body: UpdateMenuDto, @CurrentUser() user: IdentityPayload) {
