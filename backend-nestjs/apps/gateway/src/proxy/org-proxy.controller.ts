...
export class OrganizationProxyController {
  ...
  async list(@Query() query: any, @CurrentUser() user: IdentityPayload) {
    const res = await firstValueFrom(this.authClient.send({ cmd: 'org_list' }, { ...query, identity: user }));
  ...
  async create(@Body() body: any, @CurrentUser() user: IdentityPayload) {
    return firstValueFrom(this.authClient.send({ cmd: 'org_create' }, { ...body, identity: user }));
  }
