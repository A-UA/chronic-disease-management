...
export class TenantProxyController {
  ...
  async list(@Query() query: any, @CurrentUser() user: IdentityPayload) {
    const res = await firstValueFrom(this.authClient.send({ cmd: 'tenant_list' }, { ...query, identity: user }));
  ...
  async create(@Body() body: any, @CurrentUser() user: IdentityPayload) {
    return firstValueFrom(this.authClient.send({ cmd: 'tenant_create' }, { ...body, identity: user }));
  }
