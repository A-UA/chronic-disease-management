...
THRESHOLDS: dict[str, AlertThreshold] = {
    ...
    "blood_sugar": AlertThreshold(
        ...
        low=3.9,
        high=11.1,  # 空腹 3.9-6.1，餐后上限 11.1
        unit="mmol/L",
        description="血糖",
    ),
...
def check_metric_alert(
    ...
    if value_secondary is not None:
        if (
            threshold.high_secondary is not None
            and value_secondary > threshold.high_secondary
        ):
            level = (
                "danger"
                if value_secondary > threshold.high_secondary * 1.2
                else "warning"
            )
            alerts.append(
                HealthAlert(
        ...
        if (
            threshold.low_secondary is not None
            and value_secondary < threshold.low_secondary
        ):
            level = (
                "danger"
                if value_secondary < threshold.low_secondary * 0.8
                else "warning"
            )
            alerts.append(
                HealthAlert(
