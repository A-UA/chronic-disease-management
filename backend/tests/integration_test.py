...
async def run_test():
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        files = {"file": ("test.txt", b"This is a test document content about AI and RAG.")}
        resp = await client.post(f"{BASE_URL}/kb/{kb_id}/documents", files=files, headers=headers)
        if resp.status_code != 200:
        ...
        async with client.stream("POST", f"{BASE_URL}/chat/chat", json=chat_data, headers=headers) as response:
