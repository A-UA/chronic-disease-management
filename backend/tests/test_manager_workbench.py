...
async def test_manager_workflow():
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url=BASE_URL) as client:
        ...
        # Login Manager
        resp = await client.post("/auth/login/access-token", data={"username": manager_email, "password": "pass"})
        assert resp.status_code == 200
        ...
        resp = await client.post(f"/managers/patients/{patient_id}/suggestions", json=suggest_data, headers=manager_headers)
        assert resp.status_code == 200

        # 6. Test GET /managers/patients/{id}/suggestions
        resp = await client.get(f"/managers/patients/{patient_id}/suggestions", headers=manager_headers)
        assert resp.status_code == 200
