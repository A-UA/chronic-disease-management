...
class TestRerankerProviderFactory:
    ...
    def test_noop_provider(self):
        with patch("app.modules.rag.reranker_legacy.settings") as s:
            s.RERANKER_PROVIDER = "noop"
            from app.modules.rag.reranker_legacy import get_reranker_provider, NoopRerankerProvider
    ...
    def test_simple_provider(self):
        with patch("app.modules.rag.reranker_legacy.settings") as s:
            ...
            from app.modules.rag.reranker_legacy import get_reranker_provider, SimpleRerankerProvider
            p = get_reranker_provider()
    ...
    def test_zhipu_provider(self):
        with patch("app.modules.rag.reranker_legacy.settings") as s:
            ...
            s.RERANKER_BASE_URL = ""
            s.RERANKER_MODEL = ""
            from app.modules.rag.reranker_legacy import get_reranker_provider, OpenAICompatibleRerankerProvider
            p = get_reranker_provider()
...
class TestEmailService:
    ...
    async def test_no_smtp_logs_only(self):
        ...
        with patch("app.modules.auth.email.settings") as s:
            s.SMTP_HOST = ""
            s.PROJECT_NAME = "Test"
    ...
    async def test_smtp_send_failure(self):
        """SMTP 连接失败应返回 False"""
        with patch("app.modules.auth.email.settings") as s, \
             patch("app.modules.auth.email.smtplib") as mock_smtp:
...
class TestConversationCompress:
    ...
    def test_should_not_compress_short(self):
        from app.modules.rag.compress import should_compress
    ...
    async def test_compress_reduces_count(self):
        from app.modules.rag.compress import compress_history
