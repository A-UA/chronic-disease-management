...
async def kb_context() -> AsyncIterator[dict[str, object]]:
    ...
    async with AsyncSessionLocal() as cleanup_db:
        ...
        await cleanup_db.execute(delete(Organization).where(Organization.id == organization.id))
...
async def test_upload_document_and_enqueue_stores_real_object(
    ...
    monkeypatch.setattr(
        ...
        lambda _filename: type(
            "Parser",
            (),
            {"parse": lambda self, file_bytes, filename: ParseResult(text=file_bytes.decode(), pages=["page-1"])},
        )(),
    )
    monkeypatch.setattr(
        service_module,
...
async def test_delete_file_task_removes_real_minio_object(
    ...
    job = jobs[0]
    worker_fn = next(fn for fn in WorkerSettings.functions if fn.__name__ == job.function)
